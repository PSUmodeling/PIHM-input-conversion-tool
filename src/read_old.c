...
void read_alloc(char *filename, Model_Data DS, Control_Data * CS)
{
...

    {
        ...
        fscanf(riv_file, "%d %d", &(DS->Riv[i].LeftEle),
            &(DS->Riv[i].RightEle));
    ...
    DS->Riv_Shape =
        (river_shape *) malloc(DS->NumRivShape * sizeof(river_shape));
    for (i = 0; i < DS->NumRivShape; i++)

    {
        fscanf(riv_file, "%d", &DS->Riv_Shape[i].index);
        fscanf(riv_file, "%lf", &DS->Riv_Shape[i].depth);
        fscanf(riv_file, "%d %lf", &DS->Riv_Shape[i].interpOrd,
            &DS->Riv_Shape[i].coeff);
    }
    fscanf(riv_file, "%s %d", tempchar, &DS->NumRivMaterial);
    DS->Riv_Mat =
        (river_material *) malloc(DS->NumRivMaterial * sizeof(river_material));
    for (i = 0; i < DS->NumRivMaterial; i++)

    {
        fscanf(riv_file, "%d %lf %lf %lf %lf %lf", &DS->Riv_Mat[i].index,
            &DS->Riv_Mat[i].Rough, &DS->Riv_Mat[i].Cwr, &DS->Riv_Mat[i].KsatH,
    ...
    {
        fscanf(riv_file, "%s %d %d", DS->TSD_Riv[i].name,
            &DS->TSD_Riv[i].index, &DS->TSD_Riv[i].length);
        DS->TSD_Riv[i].TS =
            (double **)malloc((DS->TSD_Riv[i].length) * sizeof(double));
        ...
        {
            fscanf(riv_file, "%lf %lf", &DS->TSD_Riv[i].TS[j][0],
                &DS->TSD_Riv[i].TS[j][1]);
        }
    }
...

    {
        ...
        fscanf(att_file, "%d %d %d", &(DS->Ele[i].soil), &(DS->Ele[i].geol),
            &(DS->Ele[i].LC));
        fscanf(att_file, "%lf %lf %lf %lf %lf", &(DS->Ele_IC[i].interception),
            &(DS->Ele_IC[i].snow), &(DS->Ele_IC[i].surf),
...

    {
        ...
        fscanf(soil_file, "%lf %lf %lf", &(DS->Geol[i].vAreaF),
...

    {
        ...
        fscanf(lc_file, "%lf %lf", &(DS->LandC[i].Rmin),
            &(DS->LandC[i].Rs_ref));
        fscanf(lc_file, "%lf %lf", &(DS->LandC[i].Albedo),
            &(DS->LandC[i].VegFrac));
...

    {
...

        {
            ...
            DS->TSD_EleBC[i].TS =
                (double **)malloc((DS->TSD_EleBC[i].length) * sizeof(double));
    ...
    fscanf(para_file, "%d %d %d %d", &(CS->gwD), &(CS->surfD), &(CS->snowD),
        &(CS->rivStg));
    ...
    fscanf(para_file, "%d %d %d %d", &(CS->gwDInt), &(CS->surfDInt),
        &(CS->snowDInt), &(CS->rivStgInt));
    fscanf(para_file, "%d %d %d %d", &(CS->RechInt), &(CS->IsDInt),
        &(CS->usDInt), &(CS->etInt));
    ...
    fscanf(para_file, "%lf %lf %lf", &(CS->InitStep), &(CS->MaxStep),
        &(CS->ETStep));
    fscanf(para_file, "%lf %lf %d", &(CS->StartTime), &(CS->EndTime),
        &(CS->outtype));
    if (CS->outtype == 0)
...
/*	printf("\nStart reading in calibration file...\n");*/
    ...
    fscanf(global_calib, "%lf %lf %lf %lf %lf", &CS->Cal.KsatH,
        &CS->Cal.KsatV, &CS->Cal.infKsatV, &CS->Cal.macKsatH,
        &CS->Cal.macKsatV);
    fscanf(global_calib, "%lf %lf %lf", &CS->Cal.infD, &CS->Cal.RzD,
        &CS->Cal.macD);
    fscanf(global_calib, "%lf %lf %lf", &CS->Cal.Porosity, &CS->Cal.Alpha,
        &CS->Cal.Beta);
    fscanf(global_calib, "%lf %lf", &CS->Cal.vAreaF, &CS->Cal.hAreaF);
    fscanf(global_calib, "%lf %lf %lf", &CS->Cal.VegFrac, &CS->Cal.Albedo,
        &CS->Cal.Rough);
    fscanf(global_calib, "%lf %lf", &CS->Cal.Prep, &CS->Cal.Temp);
    fscanf(global_calib, "%lf %lf %lf", &DS->pcCal.Et0, &DS->pcCal.Et1,
        &DS->pcCal.Et2);
    fscanf(global_calib, "%lf %lf %lf %lf", &CS->Cal.rivRough,
        &CS->Cal.rivKsatH, &CS->Cal.rivKsatV, &CS->Cal.rivbedThick);
